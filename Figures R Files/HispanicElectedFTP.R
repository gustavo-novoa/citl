...
for (batch in 1:n_batches) {
  ...
  for (i in start_idx:end_idx) {
    ...
    median_maj_hisp_c <- median(all_plans$plan_maj_hisp_c, na.rm = TRUE)
    
    # Filter to median plans and sample efficiently - fix data.table filtering
    filtered_plans <- all_plans[all_plans$plan_maj_hisp_c == median_maj_hisp_c, ]
...
reps1020 <- reps1020 %>%
  group_by(city) %>%
  summarise(
    proportion_hispanic = sum(race_est == "hispanic", na.rm = TRUE) / n_distinct(district),
    absolute_hispanic = sum(race_est == "hispanic", na.rm = TRUE) ,
    .groups = 'drop'
  )
...
cat("Adding hisp_seg_c to all_median_plans...\n")
# Create a lookup table for hisp_seg_c values by city
city_hisp_seg <- data[, .(hisp_seg_c = unique(hisp_seg_c[!is.na(hisp_seg_c)])[1]), by = city]

# Merge hisp_seg_c into all_median_plans
all_median_plans <- merge(all_median_plans, city_hisp_seg, by = "city", all.x = TRUE)
all_median_plans<-subset(all_median_plans, !is.na(all_median_plans$hisp_seg_c))
...
# Create a lookup table for city hisp proportion values by city
hisp_prop <- data[, .(prop_hisp_c = unique(prop_hisp_c[!is.na(prop_hisp_c)])[1]), by = city]

# Merge city_pop_ms into all_median_plans
all_median_plans <- merge(all_median_plans, hisp_prop, by = "city", all.x = TRUE)
all_median_plans<-subset(all_median_plans, !is.na(all_median_plans$prop_hisp_c))
...
missing_seg <- sum(is.na(all_median_plans$hisp_seg_c))
if (missing_seg > 0) {
  cat("Warning:", missing_seg, "rows have missing hisp_seg_c values\n")
}
...
available_cities <- intersect(unique(long_sim$city), unique(data$city[data$hisp_viables_c]))
...
density_plot <- ggplot(long_sim_filtered, aes(x = prop_elected, y = city_ordered)) +
  ...
  geom_segment(
    data = reps1020,
    aes(x = proportion_hispanic, xend = proportion_hispanic, 
        y = as.numeric(city_ordered) - 0, yend = as.numeric(city_ordered) + .8),
    color = "red",
    linetype = "solid",
    size = 1.2,
