...
#for(i in 1:5){
  ...
  actual_maj_hisp<-c(actual_maj_hisp,sum(agg_dists$pp_hspn/agg_dists$pop>.5)/n_distinct(agg_dists$distrct))
  actual_maj_hisp_v<-c(actual_maj_hisp_v,sum(agg_dists$vap_hsp/agg_dists$vap>.5)/n_distinct(agg_dists$distrct))
  actual_maj_hisp_c<-c(actual_maj_hisp_c,sum(agg_dists$cvp_hsp/agg_dists$cvap>.5)/n_distinct(agg_dists$distrct))
  
  actual_maj_wht<-c(actual_maj_wht,sum(agg_dists$pop_wht/agg_dists$pop>.5)/n_distinct(agg_dists$distrct))
  actual_maj_wht_v<-c(actual_maj_wht_v,sum(agg_dists$vap_wht/agg_dists$vap>.5)/n_distinct(agg_dists$distrct))
  actual_maj_wht_c<-c(actual_maj_wht_c,sum(agg_dists$cvp_wht/agg_dists$cvap>.5)/n_distinct(agg_dists$distrct))
  
  actual_maj_blk<-c(actual_maj_blk,sum(agg_dists$pp_blck/agg_dists$pop>.5)/n_distinct(agg_dists$distrct))
  actual_maj_blk_v<-c(actual_maj_blk_v,sum(agg_dists$vp_blck/agg_dists$vap>.5)/n_distinct(agg_dists$distrct))
  actual_maj_blk_c<-c(actual_maj_blk_c,sum(agg_dists$cvp_blc/agg_dists$cvap>.5)/n_distinct(agg_dists$distrct))
  
  actual_maj_asn<-c(actual_maj_asn,sum(agg_dists$pop_asn/agg_dists$pop>.5)/n_distinct(agg_dists$distrct))
  actual_maj_asn_v<-c(actual_maj_asn_v,sum(agg_dists$vap_asn/agg_dists$vap>.5)/n_distinct(agg_dists$distrct))
  actual_maj_asn_c<-c(actual_maj_asn_c,sum(agg_dists$cvap_sn/agg_dists$cvap>.5)/n_distinct(agg_dists$distrct))
  ...
  plans <- plans %>%
    group_by(district) %>%
    mutate(
      majority_white = if_else(pct_white >= 0.5, 1, 0),
      majority_white_v = if_else(pct_white_v >= 0.5, 1, 0),
      majority_white_c = if_else(pct_white_c >= 0.55, 1, 0),
      
      majority_black = if_else(pct_black >= 0.5, 1, 0),
      majority_black_v = if_else(pct_black_v >= 0.5, 1, 0),
      majority_black_c = if_else(pct_black_c >= 0.5, 1, 0),
      
      majority_hisp = if_else(pct_hisp >= 0.5, 1, 0),
      majority_hisp_v = if_else(pct_hisp_v >= 0.5, 1, 0),
      majority_hisp_c = if_else(pct_hisp_c >= 0.5, 1, 0),
      
      majority_asian = if_else(pct_asian >= 0.5, 1, 0),
      majority_asian_v = if_else(pct_asian_v >= 0.5, 1, 0),
      majority_asian_c = if_else(pct_asian_c >= 0.5, 1, 0),
      
      majority_nonwhite = if_else(pct_nonwhite >= 0.5, 1, 0),
      majority_nonwhite_v = if_else(pct_nonwhite_v >= 0.5, 1, 0),
      majority_nonwhite_c = if_else(pct_nonwhite_c >= 0.5, 1, 0)
    ) %>%
    ungroup()
...
df_sums<-cbind(df_sums, actual_maj_asn, actual_maj_asn_c, actual_maj_asn_v, actual_maj_blk,
               ...
               prop_hisp, prop_hisp_v, prop_hisp_c, prop_asian, prop_asian_v,prop_asian_c)
...
#save(df_sums, file='sum_stats_2010_50.RData')
